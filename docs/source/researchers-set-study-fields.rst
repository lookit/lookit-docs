...
If you want children to become ineligible for your study after doing it once, you can add the study itself to the 'must not have participated' list. However, keep in mind that as soon as the family starts a study session, the child will be treated as having 'particpated' (even if they don't finish; see :ref:`What counts as 'participated'? <what-counts-as-participated>`). If you are running an internal (Lookit) study and want to set your own custom criteria for previous participation in your study, another option is to use the protocol generator to check the previous responses for that child/study. This will allow you to check whether the child has previously completed or reached a particular point in the study, and use that information to warn families about ineligibility (see this example in the :ref:`experiment runner protocol generator documentation <elf:generators-checking-for-completion>`).
