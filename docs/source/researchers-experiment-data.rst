...
-  *withdrawn*: Whether the participant withdrew permission for viewing/use of study video (beyond consent video). If true, the video repsonses will not be available and must not be used.
...
-  *birthdate_difference*: Difference between birthdate entered in exit survey, if any, and birthdate of registered child participating. Positive values mean that the birthdate from the exit survey is LATER. Blank if no birthdate available from the exit survey.
...
-  *is_preview*: Whether this response was generated by a researcher previewing the experiment. Preview data should not be used in any actual analyses.
